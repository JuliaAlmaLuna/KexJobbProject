...
def create_interpolation_function_ecg(fname, deriveSize):
    ...
    if fname == 'Julia/ecg_folder/Pat9_3Trace.txt' or fname == 'Julia/ecg_folder/Pat10_3Trace.txt' or fname == 'Julia/ecg_folder/Pat11_3Trace.txt' or fname == 'Julia/ecg_folder/Pat27_3Trace.txt' or fname == 'Julia/ecg_folder/Pat33_3Trace.txt' or fname == 'Julia/ecg_folder/Pat35_3Trace.txt' or fname == 'Julia/ecg_folder/Pat40_3Trace.txt' or fname == 'Julia/ecg_folder/Pat42_3Trace.txt':
        f_ecg = interp1d(ecgx, -ecgy)
        print("Used upside down {}".format(fname))
    else:
        f_ecg = interp1d(ecgx, ecgy)
        print("Used normal {}".format(fname))
    min_x = ecgx[0]
...
def createVidInputsAndTargetEcgs(videoList, ecgList, div, deriveSize = 1):
    ...
    for i, video in enumerate(videoList):
        ...
        print("Shape of vid {} is {} and shape of ecg {} is {}".format(i+1, np.shape(temp), i+1 , np.shape(temp2)))
